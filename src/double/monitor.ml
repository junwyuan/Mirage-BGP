...
module Main (S: Mirage_stack_lwt.V4) = struct
  ...
  module Replay = struct
    ...
    let rec read_loop flow pfxs_count total =
      ...
      | Ok msg -> 
        count := !count + 1;
        Replay_log.debug (fun m -> m "Receive pkg %d: %s" (!count) (Bgp.to_string msg));
        (match msg with
  ...
  module Receiver = struct
    ...
    let rec plain_feed flow count total cluster_size seed =
      if count >= total then Lwt.return_unit
        ...
        let nlri = List.map (fun ip -> (ip, 24)) ips in
        let msg =  Bgp.Update { withdrawn; path_attrs; nlri } in
        Rec_log.debug (fun m -> m "%s" (Bgp.to_string msg));
        Bgp_flow.write flow msg
        >>= function
        | Error _ ->
          Rec_log.err (fun m -> m "tcp write fail");
          Lwt.fail_with "tcp write fail"
        | Ok () -> plain_feed flow (count + cluster_size) total cluster_size n_seed
    ;;
  ...
  let start_throughput_test s total seed1 seed2 =
    let min_ad_intvl = 
      match (Key_gen.speaker ()) with
      | "quagga" -> 5
      | _ -> 5
    ...
    >>= fun (flow1, _) ->
    let replay_rloop = Replay.read_loop flow1 0 total in
    (* connect to speaker2 *)
    ...
    Lwt.join [Replay.plain_feed flow1 0 total 500 seed1; Replay.plain_feed flow2 0 total 500 seed2;]
    >>= fun () ->
    let sending_time = Unix.gettimeofday () -. start_time in
    Lwt.join [replay_rloop; rec_rloop]
    >>= fun () ->
    ...
    close flow2
    >>= fun () ->
    let new_seed1 = Ip_gen.peek_next_n seed1 total in
    let new_seed2 = Ip_gen.peek_next_n seed2 total in
    Lwt.return (new_seed1, new_seed2)
  ;;

  let start s =
    let test_sizes = [1000; 10000; 50000; 100000] in
    let rec loop seed1 seed2 = function
      | [] -> Lwt.return_unit
      | hd::tl -> 
        start_throughput_test s hd seed1 seed2
        >>= fun (new_seed1, new_seed2) ->
        ...
        >>= fun () ->
        loop new_seed1 new_seed2 tl
    in
    loop (Ip_gen.default_seed) (Int32.shift_left 202_l 24) test_sizes
  ;;
end